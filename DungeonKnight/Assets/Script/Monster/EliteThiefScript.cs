...
public class EliteThiefScript : MonsterClass, IAttack, IDefence, IMultiAtt, IDefDown, IAttUp, IDamage
{
    public float att_Point { get; set; }
    public float def_Point { get; set; }
    public float def_Save { get; set; }
    public float att_Up { get; set; }
    public float def_Down { get; set; }
    ...
    public override void PattSetting()
    {
        ...
        if (buffType != BuffType.Count && buffType != BuffType.DefDown)
        {
    ...
    public void DefDown()
    {
        eff_Root.EffectSetting(EffectType.DefDown, HeroScript.Inst.gameObject);
        SoundScript.Inst.SfSoundPlay("PowerDown");
        eff_Runtime = eff_Root.eff_Runtime;
        HeroScript.Inst.BuffCheck(BuffType.DefDown, 3.0f);
        HeroScript.Inst.def_Down = 0.2f;

        if (eff_Runtime != null)
        {
            float time = GetTime(eff_Runtime, "DefDown");
            CardCtrlScript.Inst.CardReSetting();
            StartCoroutine(EffectCo(BuffType.DefDown, 0.0f, time));
        }
    }
